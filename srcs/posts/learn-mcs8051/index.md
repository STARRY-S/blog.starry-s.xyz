...
updatedDate: 2020-12-08 09:33:06
layout: post
zone: Asia/Shanghai
tags:
- 单片机
...
1. 8051使用指令`MOVX A, @R0`读外部数据存储器时，不起作用的信号是`WR`。
   ...
   8051在读外部数据存储器时，P0和P2先输出外部数据的地址，单片机发出`ALE`信号（第电平有效）将P0口输出的地址锁存在74HC573里，之后单片机发出`RD`信号（低电平有效），读取外部数据存储器（RAM）的数据到总线再传送到P0口上。最后单片机从P0口读取数据保存到A中。
